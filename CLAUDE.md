...
All branches must follow the `<git-user>/<description>` format where `<git-user>` is your Git username (e.g., `iakor/add-login-page`). Pushes from branches that don't match this pattern are blocked by the `pre-push` hook in `.githooks/`.
